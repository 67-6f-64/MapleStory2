...
bool Engine::CInputClass::Initialize(HINSTANCE hinstance, HWND hwnd, int screenWidth, int screenHeight)
{
	...
	result = m_keyboard->SetCooperativeLevel(hwnd, DISCL_FOREGROUND | DISCL_EXCLUSIVE); // 내가 원하던 독점모드
	if(FAILED(result)) 
	{ 
		return false;
	}
	...
	result = m_mouse->SetCooperativeLevel(hwnd, DISCL_FOREGROUND | DISCL_NONEXCLUSIVE); // 현재 비배제 상태
	if(FAILED(result))
	{ 
		return false;
	}
